...
if [ "$#" -ne 1 ]; then
  printf "\nUsage: ghCUR github_clone_string\n"
  printf "    Clone Upstream Repository\n\n"
  exit
fi
...
if [ $GITHUB_USER = "BreakthroughBehavioralInc" ]; then
    printf "\nError: \"BreakthroughBehavioralInc\" is an upstream repository.\n\n"
    exit 1
fi
